...
// ---------------------------------------------------------------------------
// Categories — the "document" taxonomy. Each has a warm hue within the palette.
// ---------------------------------------------------------------------------
const CATS = [
  // Colour-blind-safe categorical palette: spread across the wheel (warm gold,
  // ink-slate, teal, ember) with stepped lightness so the four pins stay
  // distinguishable under protan/deuter/tritan vision AND in greyscale, while
  // keeping the warm-archival theme (gold + ember; slate + teal echo the ink).
...
// Safe category accessor — never returns undefined (guards against stale/legacy
// category keys so a single bad value can't crash the whole page).
...
// Map home — central Hà Nội (Hồ Hoàn Kiếm / Old Quarter)
const HANOI_CENTER = [21.0305, 105.8520];

// ---------------------------------------------------------------------------
// BASEMAPS — selectable OpenStreetMap-based tile layouts (extensible: append
// new entries here and they appear in the layer switcher automatically).
// ---------------------------------------------------------------------------
const BASEMAPS = [
...
// Timeline bounds — earliest remembered year → the relocation plan horizon.
const MIN_YEAR = 1985;
const MAX_YEAR = 2045;

// Ward centres — used to auto-label a clicked point (no manual ward field).
const WARDS = [
...
// West-bank riverside wards proposed for relocation (the clearance zone)
...
// ---------------------------------------------------------------------------
// CITIES — the national network. Each spoke is its own focused map instance
// with a bounding box for auto-fit. Only Hà Nội carries a relocation zone.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// MEMORIES — the seed archive. Fictional individuals; grounded, humane voices.
// lat / lng pinned near each ward. `photo` flags a documentary placeholder.
// ---------------------------------------------------------------------------
const MEMORIES = [
...
// ---------------------------------------------------------------------------
// The national network — humane, fictional voices pinned to real places in
// other Vietnamese cities. Same idea: memory as a quiet record of change.
// ---------------------------------------------------------------------------
...
// Real client-side image compression via canvas → JPEG data URL.
...
// Whether a memory's date is fuzzy/approximate ("circa" / "Khoảng" / "around").
function isApprox(m) {
  return /khoảng|ước/i.test(m.date || "") || /circa|approx|around|before/i.test(m.dateEn || "");
}

// Distance between two {lat,lng} points (degrees — fine for nearby ranking).
function dist(a, b) {
  return Math.hypot(a.lat - b.lat, (a.lng - b.lng) * Math.cos((a.lat * Math.PI) / 180));
}

// Format a lat/lng pair for the "document" chrome.
...
/* ===========================================================================
   ...
   The archive stays human-readable; researchers need it machine-tractable.
   These helpers turn each memory into structured, queryable, exportable data
   WITHOUT mutating the seed — derivations are pure functions of a memory.
   =========================================================================== */

// --- numeric date {year, month|null, day|null} -----------------------------
// User submissions carry explicit numeric month/day; seed memories are parsed
// from their English date string ("14 March 2026", "February 2026", "circa 1998").
function parseMemoryDate(m) {
  ...
  if (month != null) {
    const dm = s.match(/\b(\d{1,2})\s+[A-Za-z]/); // leading day before month name
    if (dm) day = +dm[1];
  }
  return { year, month, day };
}

// Inclusive integer bounds [lo,hi] of a (possibly fuzzy) date, for overlap tests.
// e.g. year-only 1998 → [19980101, 19981231]; "March 2026" → [20260301,20260331].
...
// Seed testimonies are volunteer-reviewed → verified. User submissions are
// pending review → unverified (and not yet anchored to Arweave).
...
// Deterministic pseudo Arweave transaction id (43-char base64url) — stands in
// for the content hash that anchors a verified testimony to permanent storage.
...
// ISO-8601 submission timestamp. User memories carry a real epoch; seed
// timestamps are synthesised from their date so the column is never empty.
...
// --- tags ------------------------------------------------------------------
// Tags are derived (category · ward · provenance · content keywords) so the
// Boolean text search has real metadata to query without hand-tagging 22 rows.
...
// Lower-cased haystack for Boolean search: testimony (both languages) + ward +
// category labels + derived tags. ("text content, tags, and captions")
...
// --- geometry for the spatial radius / polygon tool ------------------------
...
// Whether a memory falls inside the active spatial query shape.
