...
- **Used to permanently skip the Wayback/archive.org link.** ArchiveBox's own built-in Wayback submission is deliberately disabled (`SAVE_ARCHIVE_DOT_ORG=False` above) because `archive-worker`'s explicit `wayback.js` call already does it for every queued job — running both would hit archive.org's rate-limited Save-Page-Now API twice per capture for no benefit. But that explicit call only happens for jobs `archive-worker` actually processes, so anything added directly in the ArchiveBox admin used to never get a Wayback link at all.
  - **Fixed by a reconcile sweep** (`worker/reconcile.js`, runs every `RECONCILE_INTERVAL_MS` — default 5 min): it lists every successfully-captured URL in ArchiveBox's index, and for any URL with no `wayback_url` recorded anywhere in `archives`, submits it to Wayback and inserts a `case_id: NULL` row (nullable by design — see `POST /api/archive` — for materials not tied to a specific case) so it still shows up with both links. This is a safety net, not a replacement for submitting through the app — prefer `POST /api/archive` so materials get linked to their case.
